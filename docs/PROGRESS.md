...
Обновлять после каждого агентского этапа в его назначенной ветке. Агент ставит «на проверке» и добавляет доказательства; «принято» ставит и коммитит координатор после проверки критериев из [EXECUTION_PLAN.md](../EXECUTION_PLAN.md). Следующая ветка получает только принятый коммит. Не стирать обнаруженные блокеры без записи решения.

Допустимые состояния: `не начато`, `в работе`, `на проверке`, `на доработке`, `принято`, `блокировано`.

| ID | Статус | Ответственный | Ветка | Принятый коммит / артефакты | Проверки и доказательства | Блокер / решение |
| --- | --- | --- | --- | --- | --- | --- |
| D01 | не начато | Дизайн-агент → координатор | `codex/d01-design` | — | — | — |
...
| --- | --- | --- |
| OpenRouter API key | Живой тест перед P09; при деплое — настройка хостинга | Не передан агенту; локальный макет позволяет работать без него до финальной проверки. |
