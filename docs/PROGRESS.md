...
| C06 | на проверке | Клиентский поток → координатор | `codex/c06-streaming` | `9835d8c`; `app/chat-experience.tsx`, `app/chat-stream.mjs`, декларация типов, `app/chat-types.ts`, тесты парсера/mock, `docs/TESTING.md` | `npm run test` — 21/21; `npm run lint`, `npm run typecheck`, `npm run build`, `git diff --check` — успешно. Браузер/mock: ожидание первого чанка, Stop после `Привет`, сохранённый частичный текст без поздних дельт, следующий запрос после Stop, Esc до первого чанка | [PR #6](https://github.com/stupidkubik/ai-chat-test/pull/6) открыт; CI-проверки для ветки не настроены | Только локальный mock; живой OpenRouter не вызывался |
...
2026-09-25 — C06 — на проверке, коммит `9835d8c`
...
Решение и следующий шаг: дождаться ревью/принятия PR #6; после merge начать Q07.
